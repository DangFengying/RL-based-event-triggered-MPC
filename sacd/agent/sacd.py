...
class SacdAgent(BaseAgent):
    ...
    def __init__(self, env, test_env, log_dir, num_steps=100000, batch_size=64,
        ...
        # Define networks.
        self.policy = CateoricalPolicy(self.env.obs_space, self.env.action_space).to(self.device)
        self.online_critic = TwinnedQNetwork(self.env.obs_space, self.env.action_space,
            dueling_net=dueling_net).to(device=self.device)
        self.target_critic = TwinnedQNetwork(self.env.obs_space, self.env.action_space,
        ...
        self.target_entropy = -np.log(1.0 / self.env.action_space) * target_entropy_ratio
    ...
    def explore(self, state):
        # Act with randomness.
        state = torch.ByteTensor(state[None, ...]).to(self.device).float()
        with torch.no_grad():
