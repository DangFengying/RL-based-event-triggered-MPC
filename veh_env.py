...
def stagecost(x, u):
    ...
    return 2.0 * (x[1] - 4 * np.sin(2 * pi / 50 * x[0])) ** 2 + 0.001 * u[0] ** 2#3.0 * u[0] ** 2
...
class vehEnv:
    ...
    def step(self, action):
        ...
        tracking_error = (next_state[1] - 4 * np.sin(2 * np.pi / 50 * np.array(next_state[0]))) ** 2
        if self.t >= self.T:
            done = True
